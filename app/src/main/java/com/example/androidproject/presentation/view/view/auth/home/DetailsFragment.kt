...
class DetailsFragment : Fragment(), DetailsView {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        bundel?.let { safeBundlt ->
            detailsPresenter.getArguments(
                safeBundlt.getString(NAME),
                safeBundlt.getString(DATA),
                safeBundlt.getInt(IMAGE_VIEW)
            )
        }
    ...
    override fun displayItemData(name: String, data: String, imageView: Int) {
        binding.detailsName.text = name
        binding.detailseDate.text = data
